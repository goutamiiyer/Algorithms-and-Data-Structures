...
int main()
{
	...
	int binary_index[1000];
	int interp_index[1000];
	...
	switch(menu_choice)
	{
		...
		case 2:
			...
			printf("Comparator array is \n");
			
			for(i = 0; i < s_number; i++) {
				binarySearchLoopcount = 0;
				binary_array[i] = binarySearch(input_number, compartor_array[i], 0, N);
				printf("binary array num %d", binary_array[i]);
				printf("\nloop count is : %d\n", binarySearchLoopcount);
			}
			
			for(i = 0; i < s_number; i++){
				interpSearchCount = 0;
				interp_array[i] = interpolationSearch(input_number, compartor_array[i],N);
				printf("Interpolation Search num %d", interp_array[i]);
				printf("\nloop count is : %d\n", interpSearchCount);
			}
